...
function M.open(history)
  if not config.options.notes.enabled then return end
    ...
    .new({}, {
      ...
      attach_mappings = function(prompt_bufnr, _)
        ...
        actions.on_selection_change(function()
          if not notes_win or not vim.api.nvim_win_is_valid(notes_win2) then return end
        ...
        actions.select_default:replace(function()
          ...
          if
            notes_win
            and vim.api.nvim_win_is_valid(notes_win2)
            and not config.options.notes.persist
          then
