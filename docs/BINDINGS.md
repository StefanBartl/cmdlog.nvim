...
`default` (`<CR>`), which inserts the selected command into the cmdline,
same as `mappings.select` on the Telescope side. No favorite toggle, no
tag, no delete, in any picker.
